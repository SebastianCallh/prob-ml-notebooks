...
w₂ $(@bind μw₂ Slider(-10:10; default=0, show_value=true))
σ $(@bind σ Slider(0.1:1.5:50; default=1, show_value=true))


Σ₁₁ $(@bind Σ₁₁ Slider(0.5:0.5:10; default=1, show_value=true))
Σ₂₂ $(@bind Σ₂₂ Slider(0.5:0.5:10; default=1, show_value=true))
"""
...
$\Sigma^{-1} + \sigma^{-2} \phi_X^T \phi_X$ is $K \times  K$. When the number of  features $K$ is smaller than the number of observations $N$ the second parametrisation is more efficient. Finally, to clear up notation, let us denote the inner products and the residual between the observations and prior predictions as
...
begin
	function posterior_w(X, y, μ, Σ, σ)
		ϕX = ϕ(X)
		invΣ = inv(Σ)
	...
	function posterior_f(X, y, μ, Σ, σ)
		ϕX = ϕ(X)
		ϕx = ϕ(xx)
		κxX = ϕx'*Σ*ϕX
		κXX = ϕX'*Σ*ϕX
...
# ╔═╡ 6f0be533-29f3-47dc-83e4-b7bdd46469b7
md"""
## Computing the posterior
Let us use now compute the posterior! Below both $p(w \vert X, y)$ and the induced $p(f \vert X, y)$ are plotted using the prior parameters specified above.
"""

# ╔═╡ 2d893066-6e1b-11eb-147a-8f5062800b8d
begin
	μₙ, Σₙ = posterior_f(X, y, μ₀, Σ₀, σ)
	#alt_μₙ, alt_Σₙ = posterior_f_alt(X, y, μ₀, Σ₀, σ)
	...
	#	ribbon = 2*sqrt.(diag(alt_Σₙ)), label = "p(f | X, y)",
	#	legend = :topleft
	#)
	
	xx_w = collect(range(0.67, 1.5; length = 1000))
	wμₙ, wΣₙ = posterior_w(X, y, μ₀, Σ₀, σ)
...
md"""
The function posterior plot show us that both identities indeed produce the same posterior. And while the posterior makes sense, it does not capture the data particularly well. There is only so much we can do with a line, after all. Perhaps more problematic is the fact that the model is very(!) confident, as seen in the posterior over the weights. There is clearly room for improvement in our modelling
...
We also saw how the resulting model is surprisingly overconfident. This can be explained by the fact that the posterior uncertainty does not depend on observations $y$, only on $x$. There is simply no information about predictions being far away from observed values that can be taken into account. 
